...
year=2016    # TO BE CHANGED as well as input name
baseDir="/home/milto/Documents/ASTARTE/ASTARTE_sample_data/level4/Sentinel-1/DESCENDING/VH/heights/perYear/"$year"/"
sattelite="Sentinel-1 Descending VH"
width=900
height=400
minWidth=3.0
...
input=$baseDir"Sentinel-1_Desc_VH_heights_all_2016.csv" # TO BE CHANGED 
...
Rscript $scriptsDir"RGraphs/RERSargs.r" $output  $output".png" "Average Seasonal Backscattered Coefficient Values in "$year" using "$sattelite" data" "Months" "Average Backscattered Coefficient (DB)" $width $height

Rscript $scriptsDir"RGraphs/RERSargs.r" $output"filtered_2decimal.csv"  $output"filtered_2decimal.csv.png" "Average Seasonal Backscattered Coefficient Values Between "$year" using "$sattelite" data" "Months" "Average Backscattered Coefficient (DB)" $width $height
