...
class GeoImage:
    ...
    def __init__(self, i_inputTif):
        ...
        #self.pixelHeight = 2
        print ("-----------------------------------")
        print ("xOrigin, yOrigin", self.xOrigin, self.yOrigin)
        print ("xmin, xmax:", self.xOrigin, self.xOrigin+self.cols*self.pixelWidth)
        print ("ymin, ymax:", self.yOrigin-self.rows*self.pixelHeight, self.yOrigin)
        print ("pixelwidth x pixelheight:", self.pixelWidth, self.pixelHeight)
        print ("cols x rows:", self.cols, self.rows)
        print ("-----------------------------------")
        self.xMax=self.xOrigin+self.cols*self.pixelWidth
        self.yMin=self.yOrigin-self.rows*self.pixelHeight
    ...
    def getPixelValuefromGeo(self, i_geoX, i_geoY): 
       if(i_geoX < self.xOrigin or i_geoX>self.xMax):
          return self.noValue
       elif(i_geoX > self.yOrigin or i_geoY<self.yMin):
          return self.noValue
